...
global out "${pc}/Data"
*/
wbopendata, indicators(FP.CPI.TOTL ) long clear country(ZAF)
sum fp_cpi_totl if year==2005
local cpi_2005 = r(mean)
sum fp_cpi_totl if year==2009
local cpi_2009= r(mean)

use "${data}/ZAF09.dta", clear
replace  weight = weight*i
replace Expenditure = (Expenditure/i)/12

local cons  Expenditure
local wt  weight
local ppp2005 4.00
...
gen y = ((`cons' * `cpi_2005')/`cpi_2009')/`ppp2005' 
