...
global out "${pc}/Data"
*/
wbopendata, indicators(FP.CPI.TOTL ) long clear country(ETH)
sum fp_cpi_totl if year==2005
local cpi_2005 = r(mean)
sum fp_cpi_totl if year==2011
local cpi_2011 = r(mean)

use "${data}/ETH11.dta", clear

* Keep only selected variables
keep nom_totcons_aeq hh_size adulteq pw
replace nom_totcons_aeq = nom_totcons_aeq /12
gen weight = pw * hh_size
 
* Generate Locals with relevant variables (PPP 2005 from ICP Publication)
local cons  nom_totcons_aeq
local wt  weight
local ppp2005 2.3
