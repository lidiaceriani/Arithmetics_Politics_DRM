...
wbopendata, indicators(FP.CPI.TOTL ) long clear country(IRQ)
sum fp_cpi_totl if year==2005
local cpi_2005 = r(mean)
sum fp_cpi_totl if year==2007
local cpi_2007= r(mean)

use "${data}/IRQ07.dta", clear
replace  weight = weight*hsize
replace pcep=pcep*1000

local cons  pcep
local wt  weight
local ppp2005 540.47

keep `cons' `wt'
di `cpi_2005'
di `cpi_2011'

gen y = ((`cons' * `cpi_2005')/`cpi_2007')/`ppp2005' 
